...
const app = express();
const imageUploadPath = path.resolve("./uploads/images/default");
const fileUploadPath = path.resolve("./uploads/files/default");
...
describe("Multer Middleware Unit Tests", () => {
  describe("Image Uploads", () => {
    it("should successfully upload a valid image file", async () => {
      const response = await request(app).post("/uploads/images").attach("file", Buffer.from("test-image"), {
        filename: "test.png",
    ...
    it("should reject unsupported file types", async () => {
      const response = await request(app).post("/uploads/images").attach("file", Buffer.from("invalid-file"), {
        filename: "test.pdf",
    ...
    it("should enforce file size limits", async () => {
      ...
      const response = await request(app).post("/uploads/images").attach("file", largeFile, {
        filename: "large.png",
        contentType: "image/png",
      });
  ...
  describe("File Uploads", () => {
    it("should successfully upload a valid PDF file", async () => {
      const response = await request(app).post("/uploads/files").attach("file", Buffer.from("test-file"), {
        filename: "test.pdf",
    ...
    it("should enforce file size limits", async () => {
      ...
      const response = await request(app).post("/uploads/files").attach("file", largeFile, {
        filename: "large.pdf",
        contentType: "application/pdf",
      });
