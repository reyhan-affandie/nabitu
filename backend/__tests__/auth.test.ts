...
describe(`${moduleName} Controller UNIT TESTS`, () => {
  ...
  it(`${testTitle} REGISTER ${name} - ${OK}:OK`, async () => {
    await register(fields, CREATED, [], {
      ...overridesData,
    });
  });
  ...
  it(`${testTitle} SEND VERIFY EMAIL ${name} - ${OK}:OK`, async () => {
    const reg = await registerFlow(overridesData, overridesUser, password);
  ...
  it(`${testTitle} SEND VERIFY EMAIL ${name} - ${BAD_REQUEST}:BAD_REQUEST`, async () => {
    await registerFlow(overridesData, overridesUser, password);
  ...
  it(`${testTitle} SEND VERIFY EMAIL ${name} - ${NOT_FOUND}:NOT_FOUND`, async () => {
    const verifyEmail = await verifyEmailFlow(overridesData, overridesUser, password);
  ...
  it(`${testTitle} VERIFY EMAIL ${name} - ${NOT_FOUND}:NOT_FOUND`, async () => {
    const reg = await registerFlow(overridesData, overridesUser, password);
    await supertest(app).post(`/api/${moduleName}/email/verify`).send({ email: reg?.body?.email }).expect(OK);
    const mockTokenShort2 = generateMockTokenRandom(reg?.body?._id, reg?.body?.parent, reg?.body?.email, reg?.body?.name, true);
  ...
  it(`${testTitle} LOGOUT ${name} - ${NOT_FOUND}:NOT_FOUND`, async () => {
    const login = await loginFlow(overridesData, overridesUser, password);
    expect(login.body.token).toBeDefined();
    const mockTokenShort2 = generateMockTokenRandom(
      new mongoose.Types.ObjectId(),
      login?.body?.user?.email,
      login?.body?.user?.name,
      true,
    );
