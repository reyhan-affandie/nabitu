...
import { getAuthUser, logoutAction } from "@/actions/auth.actions";
import { useRouter } from "next/navigation";
import { NOTFOUND_RESPONSE } from "@/constants/constants";
import { UserType } from "@/lib/types/user.type";
import { FILE_ORIGIN } from "@/constants/env";
        ...
        <ClickOutside onClick={() => setDropdownOpen(false)} className="relative">
          <Link onClick={() => setDropdownOpen(!dropdownOpen)} className="flex items-center gap-4" href="#">
            ...
            <span className="h-12 w-12 overflow-hidden rounded-full">
              <Image width={112} height={112} src={`${FILE_ORIGIN}/images/users/${user?.photo}`} className="h-full w-full object-cover" alt="User" />
            </span>
